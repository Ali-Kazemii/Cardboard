...
internal class AttachmentFragment(
    private val dcId: Long,
    private val relatedTableId: Long,
    private val listener: OnActionListener
) : BaseFragment() {

    private val viewModel by viewModel<AttachmentViewModel>()
    ...
    private fun getAttachments() {
        viewModel.getListAttachment(
            DocumentAttachmentRequest().also { request ->
    ...
    private fun setAdapter(list: MutableList<AttachmentListResponse.Result>) {
        val activity = activity ?: return
        ...
        if (list.isEmpty()) {
            ...
            rclAttachment.view?.adapter = Adapter(
                list,
                object : Adapter.OnActionListener {
                    ...
                    override fun onDownload(model: AttachmentListResponse.Result) {
                        isOnDownload = true
                        rclAttachment.actionLoading = true
    ...
    private fun saveAttachment(result: AttachmentListResponse.Result) {
        val activity = activity ?: return
        if (
    ...
    companion object {
        val TAG = "${Const.APP_NAME}: ${AttachmentFragment::class.java.simpleName}"
    }
}