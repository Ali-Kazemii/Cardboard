...
internal class CardboardAddAttachmentFragment(
        ...
        if (isValid) {
            ...
            listAttachments.forEach { item ->
                viewModel.postAttachment(
                    CardboardPostAttachmentRequest().also { request ->
                        request.dvId = 1
                        request.dcId = dcId
                        request.detId = 0
                        request.datId = 0
    ...
    companion object {
        ...
        val TAG = "${Const.APP_NAME}: ${CardboardAddAttachmentFragment::class.java.simpleName}"
    }
}