...
import kotlinx.android.synthetic.main.timeline_line.view.*
import pl.hypeapp.materialtimelineview.MaterialTimelineView

internal class Adapter(private val list:MutableList<ViewType>)
    : RecyclerView.Adapter<Adapter.TimelineViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TimelineViewHolder {
        return when (viewType) {
            ViewType.LINE -> TimelineViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.timeline_line, parent, false))
    ...
    override fun onBindViewHolder(holder: TimelineViewHolder, position: Int) {
        ...
        if(model.getViewType() == ITEM)
            ...
            holder.bindLine(model as LineModel)
    }

    inner class TimelineViewHolder(itemView: View): RecyclerView.ViewHolder(itemView){
        fun bindItem(model: ViewType){

            when(model.getTimelineType()){
                SPECIFIC -> {
                    val model = model as TypeSpecific
                ...
                CREATOR ->{
                    val model = model as TypeCreator
                    itemView.layoutUserProfile.visibility = View.GONE
                    itemView.layoutSpecific.visibility = View.GONE
