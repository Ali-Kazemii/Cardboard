...
internal class CardboardAddFastReferFragment() : CardboardBaseFragment() {
    ...
    constructor(model: CardboardFastReferResponse.Result) : this() {
        this.model = model
    }

    private val viewModel by viewModel<CardboardFastReferViewModel>()

    private var model: CardboardFastReferResponse.Result? = null
    ...
    private var listReceiver: MutableList<DynamicModel<CardboardUserListResponse.Result>> = mutableListOf()
    ...
    private fun showReceivers() {
        ...
        if (listReceiver.isNotEmpty())
            CustomChooseDialog.Builder<CardboardUserListResponse.Result>()
                .source(listReceiver) {
    ...
    private val request: CardboardPostFastReferRequest
        get() {
            return CardboardPostFastReferRequest().also { request ->
                request.userId = viewModel.userId
                request.financialYearId = viewModel.financialYear
    ...
    companion object {
        val TAG = "${Const.APP_NAME}: ${CardboardAddFastReferFragment::class.java.simpleName}"
    }
}