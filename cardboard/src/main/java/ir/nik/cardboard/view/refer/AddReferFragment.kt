...
import ir.nik.cardboard.data.network.model.request.ReferenceActionRequest
...
internal class AddReferFragment(
    ...
    override fun handleOnClickListeners() {
        ...
        layoutReceiver.setOnClickListener {
            if (processStepId != -1L)
                ReceiverDialog(wfsCrID, processStepId) { model ->
                ...
                }.show(activity.supportFragmentManager, ReceiverDialog.TAG)
            else
    ...
    private fun getProcessStep() {
        if (listProcessStep.isEmpty()) {
            ...
            viewModel.getProcessStepList(
                ProcessStepRequest().also { request ->
    ...
    private fun saveReferSettings() {
        ...
        if (isValid) {
            callback.invoke(referModel, UTTWFSModel().also { model ->
                model.rowNum = 1
    ...
    companion object {
        ...
        val TAG = "${Const.APP_NAME}: ${AddReferFragment::class.java.simpleName}"
    }
}